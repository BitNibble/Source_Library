...
// for more information read data sheet.
{
	...
	timer3_state = 0;
	atmega128()->tc3_reg->tccr3a &= ~((1 << WGM31) | (1 << WGM30));
	atmega128()->tc3_reg->tccr3b &= ~((1 << WGM33) | (1 << WGM32));
	switch(wavegenmode){ // TOP -- Update of OCRnX -- TOV Flag Set on
		...
		case 8:	// PWM Phase and Frequency Correct, ICRnA -- BOTTOM -- BOTTOM
			atmega128()->tc3_reg->tccr3b |= (1 << WGM33);
		break;
	...
	atmega128()->tc3_reg->tccr3a &= ~((3 << COM3A0) | (3 << COM3B0)| (3 << COM3C0));
	atmega128()->tc3_reg->etimsk &= ~((1 << TICIE3) | (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3) | (1 << OCIE3C));
	...
	atmega128()->tc3_reg->ocr3a = atmega128()->writehlbyte(~0);
	atmega128()->tc3_reg->ocr3b = atmega128()->writehlbyte(~0);
	atmega128()->tc3_reg->ocr3c = atmega128()->writehlbyte(~0);
...
void TIMER_COUNTER3_compoutmodeA(unsigned char compoutmode)
{
	atmega128()->tc3_reg->tccr3a &= ~(3 << COM3A0);
...
void TIMER_COUNTER3_compoutmodeB(unsigned char compoutmode)
{
	atmega128()->tc3_reg->tccr3a &= ~(3 << COM3B0);
...
void TIMER_COUNTER3_compoutmodeC(unsigned char compoutmode)
{
	atmega128()->tc3_reg->tccr3a &= ~(3 << COM3C0);
...
void TIMER_COUNTER3_compareA(uint16_t compare)
{
	atmega128()->tc3_reg->ocr3a = atmega128()->writehlbyte(compare);
}
void TIMER_COUNTER3_compareB(uint16_t compare)
{
	atmega128()->tc3_reg->ocr3b = atmega128()->writehlbyte(compare);
}
void TIMER_COUNTER3_compareC(uint16_t compare)
{
	atmega128()->tc3_reg->ocr3c = atmega128()->writehlbyte(compare);
}
uint8_t TIMER_COUNTER3_stop(void)
