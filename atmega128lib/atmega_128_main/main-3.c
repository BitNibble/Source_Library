		...
		switch(Menu){
			...
			case '1': // Main Program Menu
				...
				if (value > 1000 || value < -1000){
					publish = value / 1000;
					lcd0()->gotoxy(2,1);
					lcd0()->string_size(function.ftoa(publish, 3), 13); lcd0()->string_size("Kg", 4);
				}else{
					publish = value;
					lcd0()->gotoxy(2,1);
					lcd0()->string_size(function.ftoa(publish, ZERO), 13); lcd0()->string_size("gram", 4);
				}
				break;
			/***MENU 2***/
