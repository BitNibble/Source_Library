...
// for more information read data sheet.
{
	...
	timer1_state = 0;
	atmega128()->tc1_reg->tccr1a.reg &= ~((1 << WGM11) | (1 << WGM10));
	atmega128()->tc1_reg->tccr1b.reg &= ~((1 << WGM13) | (1 << WGM12));
	switch(wavegenmode){ // TOP -- Update of OCRnX at -- TOV Flag Set on
		...
		case 8:	// PWM Phase and Frequency Correct, ICRnA -- BOTTOM -- BOTTOM
			atmega128()->tc1_reg->tccr1b.reg |= (1 << WGM13);
		break;
	...
	atmega128()->tc1_reg->tccr1a.reg &= ~((3 << COM1A0) | (3 << COM1B0) | (3 << COM1C0));
	atmega128()->tc1_reg->timsk.reg &= ~((1 << TICIE1) | (1 << OCIE1A) | (1 << OCIE1B) | (1 << TOIE1));
	atmega128()->tc1_reg->etimsk.reg &= ~(1 << OCIE1C);
	switch(interrupt){ // ICP1  -->  PD4
		case 0:
		break;
		case 1:
			atmega128()->tc1_reg->timsk.reg |= (1 << TOIE1);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 2:
			atmega128()->tc1_reg->timsk.reg |= (1 << OCIE1A);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 3:
			atmega128()->tc1_reg->timsk.reg |= (1 << OCIE1B);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 4:
			atmega128()->tc1_reg->etimsk.reg |= (1 << OCIE1C);
		...
		break;
		case 5:
			atmega128()->tc1_reg->timsk.reg |= (1 << TICIE1);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 6:
			atmega128()->tc1_reg->timsk.reg |= (1 << OCIE1A) | (1 << TOIE1);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 7:
			atmega128()->tc1_reg->timsk.reg |= (1 << OCIE1B) | (1 << TOIE1);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 8:
			atmega128()->tc1_reg->timsk.reg |= (1 << TOIE1);
		...
		break;
		case 9:
			atmega128()->tc1_reg->timsk.reg |= (1 << TICIE1) | (1 << TOIE1);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 10:
			atmega128()->tc1_reg->timsk.reg |= (1 << OCIE1A) | (1 << OCIE1B) | (1 << TOIE1);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 11:
			atmega128()->tc1_reg->timsk.reg |= (1 << OCIE1A) | (1 << OCIE1B) | (1 << TOIE1);
		...
		break;
		case 12:
			atmega128()->tc1_reg->timsk.reg |= (1 << OCIE1A) | (1 << OCIE1B);
			atmega128()->tc1_reg->etimsk.reg |= (1 << OCIE1C);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		default:
...
void TIMER_COUNTER1_compoutmodeA(unsigned char compoutmode)
{
	atmega128()->tc1_reg->tccr1a.reg &= ~(3 << COM1A0);
...
void TIMER_COUNTER1_compoutmodeB(unsigned char compoutmode)
{
	atmega128()->tc1_reg->tccr1a.reg &= ~(3 << COM1B0);
...
void TIMER_COUNTER1_compoutmodeC(unsigned char compoutmode)
{
	atmega128()->tc1_reg->tccr1a.reg &= ~(3 << COM1C0);
