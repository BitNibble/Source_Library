...
// for more information read data sheet.
{
	...
	tc1_handle()->tccr1a.reg &= ~((1 << WGM11) | (1 << WGM10));
	tc1_handle()->tccr1b.reg &= ~((1 << WGM13) | (1 << WGM12));
	...
	tc1_handle()->tccr1a.reg &= ~((3 << COM1A0) | (3 << COM1B0) | (3 << COM1C0));
	tc1_handle()->timsk.reg &= ~((1 << TICIE1) | (1 << OCIE1A) | (1 << OCIE1B) | (1 << TOIE1));
	tc1_handle()->etimsk.reg &= ~(1 << OCIE1C);
	switch(interrupt){ // ICP1  -->  PD4
		case 0:
		break;
		case 1:
			tc1_handle()->timsk.reg |= (1 << TOIE1);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 2:
			tc1_handle()->timsk.reg |= (1 << OCIE1A);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 3:
			tc1_handle()->timsk.reg |= (1 << OCIE1B);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 4:
			tc1_handle()->etimsk.reg |= (1 << OCIE1C);
		...
		break;
		case 5:
			tc1_handle()->timsk.reg |= (1 << TICIE1);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 6:
			tc1_handle()->timsk.reg |= (1 << OCIE1A) | (1 << TOIE1);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 7:
			tc1_handle()->timsk.reg |= (1 << OCIE1B) | (1 << TOIE1);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 8:
			tc1_handle()->timsk.reg |= (1 << TOIE1);
		...
		break;
		case 9:
			tc1_handle()->timsk.reg |= (1 << TICIE1) | (1 << TOIE1);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 10:
			tc1_handle()->timsk.reg |= (1 << OCIE1A) | (1 << OCIE1B) | (1 << TOIE1);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 11:
			tc1_handle()->timsk.reg |= (1 << OCIE1A) | (1 << OCIE1B) | (1 << TOIE1);
		...
		break;
		case 12:
			tc1_handle()->timsk.reg |= (1 << OCIE1A) | (1 << OCIE1B);
			tc1_handle()->etimsk.reg |= (1 << OCIE1C);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		default:
	...
	tc1_handle()->ocr1a = writehlbyte(~0);
	tc1_handle()->ocr1b = writehlbyte(~0);
	tc1_handle()->ocr1c = writehlbyte(~0);
	
	atmega128_tc1.handle = tc1_handle();
...
void TIMER_COUNTER1_compareA(uint16_t compare)
{
	tc1_handle()->ocr1a = writehlbyte(compare);
}
void TIMER_COUNTER1_compareB(uint16_t compare)
{
	tc1_handle()->ocr1b = writehlbyte(compare);
}
void TIMER_COUNTER1_compareC(uint16_t compare)
{
	tc1_handle()->ocr1c = writehlbyte(compare);
}
uint8_t TIMER_COUNTER1_stop(void)
