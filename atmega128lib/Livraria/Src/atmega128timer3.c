...
// for more information read data sheet.
{
	...
	timer3_state = 0;
	atmega128()->tc3_reg->tccr3a.reg &= ~((1 << WGM31) | (1 << WGM30));
	atmega128()->tc3_reg->tccr3b.reg &= ~((1 << WGM33) | (1 << WGM32));
	switch(wavegenmode){ // TOP -- Update of OCRnX -- TOV Flag Set on
		...
		case 8:	// PWM Phase and Frequency Correct, ICRnA -- BOTTOM -- BOTTOM
			atmega128()->tc3_reg->tccr3b.reg |= (1 << WGM33);
		break;
	...
	atmega128()->tc3_reg->tccr3a.reg &= ~((3 << COM3A0) | (3 << COM3B0)| (3 << COM3C0));
	atmega128()->tc3_reg->etimsk.reg &= ~((1 << TICIE3) | (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3) | (1 << OCIE3C));
	switch(interrupt){ // ICP3  -->  PE7
		case 0:
		break;
		case 1:
			atmega128()->tc3_reg->etimsk.reg |= (1 << TOIE3);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 2:
			atmega128()->tc3_reg->etimsk.reg |= (1 << OCIE3A);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 3:
			atmega128()->tc3_reg->etimsk.reg |= (1 << OCIE3B);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 4:
			atmega128()->tc3_reg->etimsk.reg |= (1 << OCIE3C);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 5:
			atmega128()->tc3_reg->etimsk.reg |= (1 << TICIE3);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 6:
			atmega128()->tc3_reg->etimsk.reg |= (1 << OCIE3A) | (1 << TOIE3);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 7:
			atmega128()->tc3_reg->etimsk.reg |= (1 << OCIE3B) | (1 << TOIE3);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 8:
			atmega128()->tc3_reg->etimsk.reg |= (1 << TOIE3) | (1 << OCIE3C);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 9:
			atmega128()->tc3_reg->etimsk.reg |= (1 << TICIE3) | (1 << TOIE3);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 10:
			atmega128()->tc3_reg->etimsk.reg |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 11:
			atmega128()->tc3_reg->etimsk.reg |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3) | (1 << OCIE3C);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 12:
			atmega128()->tc3_reg->etimsk.reg |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << OCIE3C);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		default:
		break;
	}
	atmega128()->tc3_reg->ocr3a = writehlbyte(~0);
	atmega128()->tc3_reg->ocr3b = writehlbyte(~0);
	atmega128()->tc3_reg->ocr3c = writehlbyte(~0);
...
void TIMER_COUNTER3_compoutmodeA(unsigned char compoutmode)
{
	atmega128()->tc3_reg->tccr3a.reg &= ~(3 << COM3A0);
...
void TIMER_COUNTER3_compoutmodeB(unsigned char compoutmode)
{
	atmega128()->tc3_reg->tccr3a.reg &= ~(3 << COM3B0);
...
void TIMER_COUNTER3_compoutmodeC(unsigned char compoutmode)
{
	atmega128()->tc3_reg->tccr3a.reg &= ~(3 << COM3C0);
...
void TIMER_COUNTER3_compareA(uint16_t compare)
{
	atmega128()->tc3_reg->ocr3a = writehlbyte(compare);
}
void TIMER_COUNTER3_compareB(uint16_t compare)
{
	atmega128()->tc3_reg->ocr3b = writehlbyte(compare);
}
void TIMER_COUNTER3_compareC(uint16_t compare)
{
	atmega128()->tc3_reg->ocr3c = writehlbyte(compare);
}
uint8_t TIMER_COUNTER3_stop(void)
