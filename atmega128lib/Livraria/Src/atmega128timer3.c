...
// for more information read data sheet.
{
	...
	timer3_state = 0;
	atmega128()->tc3_handle->tccr3a.reg &= ~((1 << WGM31) | (1 << WGM30));
	atmega128()->tc3_handle->tccr3b.reg &= ~((1 << WGM33) | (1 << WGM32));
	switch(wavegenmode){ // TOP -- Update of OCRnX -- TOV Flag Set on
		...
		case 8:	// PWM Phase and Frequency Correct, ICRnA -- BOTTOM -- BOTTOM
			atmega128()->tc3_handle->tccr3b.reg |= (1 << WGM33);
		break;
	...
	atmega128()->tc3_handle->tccr3a.reg &= ~((3 << COM3A0) | (3 << COM3B0)| (3 << COM3C0));
	atmega128()->tc3_handle->etimsk.reg &= ~((1 << TICIE3) | (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3) | (1 << OCIE3C));
	switch(interrupt){ // ICP3  -->  PE7
		case 0:
		break;
		case 1:
			atmega128()->tc3_handle->etimsk.reg |= (1 << TOIE3);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 2:
			atmega128()->tc3_handle->etimsk.reg |= (1 << OCIE3A);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 3:
			atmega128()->tc3_handle->etimsk.reg |= (1 << OCIE3B);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 4:
			atmega128()->tc3_handle->etimsk.reg |= (1 << OCIE3C);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 5:
			atmega128()->tc3_handle->etimsk.reg |= (1 << TICIE3);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 6:
			atmega128()->tc3_handle->etimsk.reg |= (1 << OCIE3A) | (1 << TOIE3);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 7:
			atmega128()->tc3_handle->etimsk.reg |= (1 << OCIE3B) | (1 << TOIE3);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 8:
			atmega128()->tc3_handle->etimsk.reg |= (1 << TOIE3) | (1 << OCIE3C);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 9:
			atmega128()->tc3_handle->etimsk.reg |= (1 << TICIE3) | (1 << TOIE3);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 10:
			atmega128()->tc3_handle->etimsk.reg |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 11:
			atmega128()->tc3_handle->etimsk.reg |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3) | (1 << OCIE3C);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 12:
			atmega128()->tc3_handle->etimsk.reg |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << OCIE3C);
			atmega128()->cpu_handle->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		default:
		break;
	}
	atmega128()->tc3_handle->ocr3a = writehlbyte(~0);
	atmega128()->tc3_handle->ocr3b = writehlbyte(~0);
	atmega128()->tc3_handle->ocr3c = writehlbyte(~0);
...
// External clock source on Tn pin. Clock on rising edge; default - clk T 0 S /1024 (From prescaler).
{
	if(!timer3_state){ // one shot
		atmega128()->tc3_handle->tccr3b.reg &= ~(7 << CS30); // No clock source. (Timer/Counter stopped)
		switch(prescaler){
			//case 0: // No clock source. (Timer/Counter stopped)
			//break;
			case 1: // clkI/O/1 (No prescaler)
				atmega128()->tc3_handle->tccr3b.reg |= (1 << CS30);
			break;
			case 8: // clkI/O/8 (From prescaler)
				atmega128()->tc3_handle->tccr3b.reg |= (1 << CS31);
			break;
			case 64: // clkI/O/64 (From prescaler)
				atmega128()->tc3_handle->tccr3b.reg |= (3 << CS30);
			break;
			case 256: // clkI/O/256 (From prescaler)
				atmega128()->tc3_handle->tccr3b.reg |= (1 << CS32);
			break;
			case 1024: // clkI/O/1024 (From prescaler)
				atmega128()->tc3_handle->tccr3b.reg |= (5 << CS30);
			break;
			case 6: // External clock source on Tn pin. Clock on falling edge [PE6]
				atmega128()->tc3_handle->tccr3b.reg |= (6 << CS30);
			break;
			case 7: // External clock source on Tn pin. Clock on rising edge [PE6]
				atmega128()->tc3_handle->tccr3b.reg |= (7 << CS30);
			break;
			default:
				atmega128()->tc3_handle->tccr3b.reg |= (5 << CS30);
			break;
...
void TIMER_COUNTER3_compareA(uint16_t compare)
{
	atmega128()->tc3_handle->ocr3a = writehlbyte(compare);
}
void TIMER_COUNTER3_compareB(uint16_t compare)
{
	atmega128()->tc3_handle->ocr3b = writehlbyte(compare);
}
void TIMER_COUNTER3_compareC(uint16_t compare)
{
	atmega128()->tc3_handle->ocr3c = writehlbyte(compare);
}
uint8_t TIMER_COUNTER3_stop(void)
