...
// for more information read data sheet.
{
	...
	timer3_state = 0;
	tc3_handle()->tccr3a.reg &= ~((1 << WGM31) | (1 << WGM30));
	tc3_handle()->tccr3b.reg &= ~((1 << WGM33) | (1 << WGM32));
	...
	tc3_handle()->tccr3a.reg &= ~((3 << COM3A0) | (3 << COM3B0)| (3 << COM3C0));
	tc3_handle()->etimsk.reg &= ~((1 << TICIE3) | (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3) | (1 << OCIE3C));
	switch(interrupt){ // ICP3  -->  PE7
		case 0:
		break;
		case 1:
			tc3_handle()->etimsk.reg |= (1 << TOIE3);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 2:
			tc3_handle()->etimsk.reg |= (1 << OCIE3A);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 3:
			tc3_handle()->etimsk.reg |= (1 << OCIE3B);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 4:
			tc3_handle()->etimsk.reg |= (1 << OCIE3C);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 5:
			tc3_handle()->etimsk.reg |= (1 << TICIE3);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 6:
			tc3_handle()->etimsk.reg |= (1 << OCIE3A) | (1 << TOIE3);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 7:
			tc3_handle()->etimsk.reg |= (1 << OCIE3B) | (1 << TOIE3);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 8:
			tc3_handle()->etimsk.reg |= (1 << TOIE3) | (1 << OCIE3C);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 9:
			tc3_handle()->etimsk.reg |= (1 << TICIE3) | (1 << TOIE3);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 10:
			tc3_handle()->etimsk.reg |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 11:
			tc3_handle()->etimsk.reg |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3) | (1 << OCIE3C);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 12:
			tc3_handle()->etimsk.reg |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << OCIE3C);
			cpu_handle()->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		default:
		break;
	}
	tc3_handle()->ocr3a = writehlbyte(~0);
	tc3_handle()->ocr3b = writehlbyte(~0);
	tc3_handle()->ocr3c = writehlbyte(~0);
	
	atmega128_tc3.handle = tc3_handle();
...
void TIMER_COUNTER3_compareA(uint16_t compare)
{
	tc3_handle()->ocr3a = writehlbyte(compare);
}
void TIMER_COUNTER3_compareB(uint16_t compare)
{
	tc3_handle()->ocr3b = writehlbyte(compare);
}
void TIMER_COUNTER3_compareC(uint16_t compare)
{
	tc3_handle()->ocr3c = writehlbyte(compare);
}
uint8_t TIMER_COUNTER3_stop(void)
