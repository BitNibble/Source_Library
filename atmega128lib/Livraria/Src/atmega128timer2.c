...
// interrupt: off; overflow; output compare; both; default - non.
{
	...
	timer2_state = 0;
	atmega128()->tc2_reg->tccr2 &= ~((1 << WGM20) | (1 << WGM21));
	switch(wavegenmode){ // TOP -- Update of OCR2 at -- TOV0 Flag Set on
		...
		case 1: // PWM Phase Correct, 0xFF -- TOP -- BOTTOM
			atmega128()->tc2_reg->tccr2 |= (1 << WGM20);
		break;
		case 2: // CTC, OCR2 -- Immediate -- MAX
			atmega128()->tc2_reg->tccr2 |= (1 << WGM21);
		break;
	...
	atmega128()->tc2_reg->timsk &= ~((1 << TOIE2) | (1 << OCIE2));
	switch(interrupt){
		case 0:
		break;
		case 1:
			atmega128()->tc2_reg->timsk |= (1 << TOIE2);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 2:
			atmega128()->tc2_reg->timsk |= (1 << OCIE2);
			atmega128()->cpu_reg->sreg.reg |= 1 << GLOBAL_INTERRUPT_ENABLE;
		break;
		case 3:
			atmega128()->tc2_reg->timsk |= (1 << TOIE2);
...
void TIMER_COUNTER2_compare(unsigned char compare)
{
	atmega128()->tc2_reg->ocr2 = compare;
}
uint8_t TIMER_COUNTER2_stop(void)
