...
{
	timer1_state = 0;
	tc1_handle()->tccr1a &= ~((1 << WGM11) | (1 << WGM10));
	tc1_handle()->tccr1b &= ~((1 << WGM13) | (1 << WGM12));
	switch(wavegenmode){
		case 0: // Normal
		break;
		case 1: // PWM, Phase Correct, 8-bit
			tc1_handle()->tccr1a |= (1 << WGM10);
		break;
		case 2:	// PWM, Phase Correct, 9-bit
			tc1_handle()->tccr1a |= (1 << WGM11);
		break;
		case 3:	// PWM, Phase Correct, 10-bit
			tc1_handle()->tccr1a |= (1 << WGM11) | (1 << WGM10);
		break;
		case 4:	// CTC
			tc1_handle()->tccr1b |= (1 << WGM12);
		break;
	...
	tc1_handle()->tccr1a &= ~((3 << COM1A0) | (3 << COM1B0));
	tc_imask_handle()->timsk1 &= ~((1 << ICIE1)  | (1 << OCIE1B) | (1 << OCIE1A) | (1 << TOIE1));
	switch(interrupt){
		case 0:
		break;
		case 1:
			tc_imask_handle()->timsk1 |= (1 << TOIE1);
			cpu_handle()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 2:
			tc_imask_handle()->timsk1 |= (1 << OCIE1A);
			cpu_handle()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 3:
			tc_imask_handle()->timsk1 |= (1 << OCIE1B);
			cpu_handle()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 4:
			tc_imask_handle()->timsk1 |= (1 << ICIE1);
			cpu_handle()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 5:
			tc_imask_handle()->timsk1 |= (1 << OCIE1A) | (1 << OCIE1B);
			cpu_handle()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 6:
			tc_imask_handle()->timsk1 |= (1 << OCIE1A) | (1 << TOIE1);
			cpu_handle()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 7:
			tc_imask_handle()->timsk1 |= (1 << OCIE1B) | (1 << TOIE1);
			cpu_handle()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 8:
			tc_imask_handle()->timsk1 |= (1 << ICIE1) | (1 << TOIE1);
			cpu_handle()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 9:
			tc_imask_handle()->timsk1 |= (1 << OCIE1A) | (1 << OCIE1B) | (1 << TOIE1);
			cpu_handle()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 10:
			tc_imask_handle()->timsk1 |= (1 << ICIE1) | (1 << OCIE1A) | (1 << OCIE1B) | (1 << TOIE1);
			cpu_handle()->sreg |= (1 << GLOBAL_INTERRUPT_ENABLE);
		break;
		case 11:
			tc_imask_handle()->timsk1 |= (1 << OCIE1A) | (1 << OCIE1B);
...
void TIMER_COUNTER1_compoutmodeA(unsigned char compoutmode)
{
	...
	switch(compoutmode){ // see table 53, 54, 55 in data sheet for more information
		...
		case 1: // Reserved
			...
			portb_handle()->ddr |= (1 << 1);
			tc1_handle()->tccr1a |= (1 << COM1A0);
		break;
		...
		case 3: // Set OC1 on compare match when up-counting.
			...
			portb_handle()->ddr |= (1 << 1);
			tc1_handle()->tccr1a |= (1 << COM1A0) | (1 << COM1A1);
		break;
...
void TIMER_COUNTER1_compoutmodeB(unsigned char compoutmode)
{
	tc1_handle()->tccr1a &= ~(3 << COM1B0);
	switch(compoutmode){ // see table 53, 54, 55 in data sheet for more information
		case 0: // Normal port operation, OC1 disconnected.
		break;
		case 1: // Reserved
				// Toggle OC1 on compare match
			portb_handle()->ddr |= (1 << 2);
			tc1_handle()->tccr1a |= (1 << COM1B0);
		break;
		case 2: // Clear OC1 on compare match when up-counting.
				// Set OC1 on compare
				// match when down counting.
		...
		case 3: // Set OC1 on compare match when up-counting.
				...
				// Set OC1 on compare match
			portb_handle()->ddr |= (1 << 2);
			tc1_handle()->tccr1a |= (1 << COM1B0) | (1 << COM1B1);
		break;
		default: // Normal port operation, OC1 disconnected.
		break;
	}
...
void TIMER_COUNTER1_compareA(uint16_t compare)
{
	tc1_compare_handle()->ocr1a = writehlbyte(compare);
}
void TIMER_COUNTER1_compareB(uint16_t compare)
{
	tc1_compare_handle()->ocr1b = writehlbyte(compare);
}
void TIMER_COUNTER1_stop(void)
// stops timer by setting prescaler to zero
{
	tc1_handle()->tccr1b &= ~(7 << CS10); // No clock source. (Timer/Counter stopped)
	tc1_handle()->tcnt1 = writehlbyte(0X0000);
	timer1_state = 0;
}
