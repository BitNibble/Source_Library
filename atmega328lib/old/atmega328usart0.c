...
USART0 USART0enable(uint32_t baud, unsigned int FDbits, unsigned int Stopbits, unsigned int Parity )
{
	...
	atmega328.usart0.reg->ucsr0b = (1 << RXCIE0) | (1 << RXEN0) | (1 << TXEN0);
...
{
	uint16_t timeout;
	atmega328.usart0.reg->ucsr0b |= 1 << UDRIE0;
...
{ // USART, RX Complete Handler
    ...
    bit_8 = atmega328.usart0.reg->ucsr0b;
    bit_8 = 0x01 & (bit_8 >> 1);
...
uint8_t USART0ReadErrors(void)
{
	return atmega328.readreg(UCSR0A,3,2);
}
void USART0ClearErrors(void)
{
	atmega328.setreg(&UCSR0A,3,2,0);
}
void USART0DoubleTransmissionSpeed(void)
{
	atmega328.setreg(&UCSR0A,4,1,1);
}
