...
{	
    ...
    for (window = 0, menu = 1; TRUE; ) // Looping
    {
		...
		if(!window){ // Loop preamble
			lcd.reboot();
			input = ( atmega328()->portc_reg->pin & 0xF0 ) | readreg( atmega328()->portb_reg->pin, 4, 4);
...
void PORTINIT(void)
{
	writereg(&atmega328()->portb_reg->ddr,2,4,0);
	writereg(&atmega328()->portb_reg->port,2,4,3);
	writereg(&atmega328()->portc_reg->ddr,2,4,0);
	writereg(&atmega328()->portc_reg->port,2,4,3);
	writereg(&atmega328()->portd_reg->ddr,2,2,0);
	writereg(&atmega328()->portd_reg->port,2,2,3);
}
